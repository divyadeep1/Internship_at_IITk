...
## X shape:
#vertices = [[-100,-100],[-100,100],[0,0],[100,-100],[100,100]]
#edges = [[2],[2],[0,3,4,1],[2],[2]]

## L shape:
vertices = [[100,100],[150,100],[100,50]]
edges = [[1,2],[0],[0]]
...
def draw_lines(lines):
	...
	for line in lines:
		img = cv2.line(img,(int(line[0]),int(line[1])), (int(line[2]),int(line[3])), (random.randint(0,255),random.randint(0,255),random.randint(0,255)),1)
...
def extrude_1d_walls_in_3d(file_name):
	...
	for i in range(len(edges)):
		for j in edges[i]:
			...
			#offset = weights[i][edges[i].index(j)]*d/2*normal_above
			offset = d/2*normal_above
			pts = np.array([p1,p2])
...
#				#print("i,double_indices[i]: ",i,double_indices[i])
	...
	#print("New vertices with normals: ")
	#for i in range(len(new_vertices)):
		#print("Vertex, normal : ", new_vertices[i], normals[i])

	#print("New edges: ")
	#for i in new_edges:
		#print(i)

	#print("Double indexed edges:")
	#for i in double_indices:
		#print(np.array(i).tolist())

	#print("Number of edges before capping: ", len(new_edges))
	...
	for i,e in enumerate(new_edges):
		a = [new_vertices[i][0], new_vertices[i][1], new_vertices[e[0]][0], new_vertices[e[0]][1]]
		lines.append(a)
	draw_lines(lines)
	##print(data)
...
if __name__ == "__main__":
	...
	extrude_1d_walls_in_3d("testing")
